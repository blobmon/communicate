...
def run():
    ...
    cur.execute("SELECT * FROM threads WHERE delete_status>0 AND now() - bump_ts > INTERVAL '7 day' ")
    ...
    #deleting inactive users now
    cur.execute("DELETE FROM users WHERE now() - last_activity_ts > INTERVAL '50 day' ")

    #also delete old login attempt logs
    cur.execute("DELETE FROM login_attempt_log WHERE now() - ts > INTERVAL '7 day' ")
